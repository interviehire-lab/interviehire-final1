...
- **Auth:** JWT httpOnly cookie (required) + _verify_job_access.
- **Path params:** `job_id`:UUID — the job id
- **Query params:** `source`:ApplicantSource (optional, legacy) — `career_page|bulk_upload|direct_link|scheduled|ats|functional|exit`. Accepted and enum-validated for backwards compatibility, but ignored by this route; it never selects or advances a pipeline stage.
...
Notes: `response_model=List[ApplicantOut]`. Saves to `uploads/resumes/`. `DEEPSEEK_API_KEY` is read from env for `parse_resume_with_deepseek`. Deduplication matches an existing applicant first by lowercase email (ignoring `@candidate.io` dummies), then by lowercase name (ignoring `Candidate`). For a match, the route updates `resume_url`, usable `resume_text`, and only placeholder/unset name, email, or phone values; it does **not** mutate the existing applicant's `source`, `screening_status`, or `functional_status`. A new applicant gets email fallback `candidate.<hex>@candidate.io`, phone `+1 555-0199`, `source="bulk_upload"`, `entry_method="bulk_upload"`, `resume_analysed=false`, and both stage-status fields unset (`screening_status=null`, `functional_status=null`). The legacy `source` query value is ignored in both branches. Uploading a resume therefore never advances a candidate to recruiter screening or functional interviewing; stage advancement remains an explicit recruiter action. Broadcasts a WebSocket update.
