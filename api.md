...
- **2026-09-09** — **Voice-safe interview question length for generated blueprints.** **POST `/api/company/questions/generate`** keeps the same request/response schema, but both its DeepSeek prompt and deterministic fallback now limit each main spoken question to one idea and at most 26 words; extra probes belong in later conversational turns. Backend→engine provisioning invoked by **POST `/api/jobs/{job_id}/test-session`**, **POST `/api/jobs/applicants/{applicant_id}/schedule`**, and invite provisioning now also shortens overlong, unedited structured blueprint questions from their saved `targetRequirement`/`competency` before upserting engine `Question` rows. **POST `/api/interview/sessions/:id/start`** and the conversational director use the same runtime-safe effective-question list, so sessions synced before this patch are shortened immediately as well. Questions whose guidance carries `edited: true`, and legacy plain-text guidance that cannot be identified safely as generated, remain untouched. No request/response fields or status codes changed.
- **2026-09-09** — **Removed software-engineering assumptions from JD extraction, question generation, interview start, and backend→engine role sync.** **POST `/api/jobs/extract-jd`** keeps the same request/response schema and provider order, but every provider is now explicitly constrained to source-grounded competencies; when all configured providers fail (or none is configured), it returns the extracted title/raw JD with empty question/criteria/topic arrays instead of fabricating a software role, Python/React skills, or system-design/coding questions. **POST `/api/company/questions/generate`** keeps the same schema and still works without a model key, but its deterministic fallback now derives questions/rubrics from clauses in that request's `jobDescription` (or the submitted role title when the description is empty) rather than returning a generic software-leaning question. **POST `/api/interview/sessions/:id/start`** keeps the same response shape, but `initialQuestion` now comes from the role's active authored questions or a role/JD-derived fallback; the former hardcoded software-engineering fallback was removed. Internally, applicant/session provisioning now derives `JobRole.primaryCriteria`/`secondaryCriteria` from saved resume criteria, functional topic names, and authored rubric pins rather than defaulting all roles to coding/system design. No endpoint fields or status codes changed.
- **2026-09-07** — **`POST /api/public/reschedule/{token}` gains `skip_notification` (request, optional bool, default false) and `applicant_id`/`job_id` (response) — powering a new "Start interview now" action on `app/reschedule`.** Full details in the endpoint reference below. Also: `app/reschedule`'s date/time picker was swapped from `react-datepicker` (see the entry directly below — introduced and then replaced the same day) to a custom calendar+native-`<input type="time">` component (`DateTimeField.tsx`, styled port of `src/dashboard/resume-analysis.ts`'s `createDateTimePicker`), so time selection is no longer restricted to 15-minute increments; and a route-scoped CSS reset (`app/reschedule/layout.tsx` + `reschedule-reset.css`) fixed a white sliver around the page edges (`body`'s default UA margin/white background showing through, since `reschedule.module.css`'s CSS-Modules scoping can't target a bare `body`/`:root` selector itself). No backend routes besides `/reschedule/{token}` changed.
- **2026-09-07** — **The candidate reschedule link's URL format changed from `{FRONTEND_URL}/reschedule.html?token=...` to `{FRONTEND_URL}/reschedule?token=...`.** `dashboard/public/reschedule.html` (a static page) was replaced by a proper Next.js App Router route at `dashboard/app/reschedule/page.tsx`, using `react-datepicker` (timezone-locked to IST via its `timeZone` prop) instead of a bare `<input type="datetime-local">`. The route consumes the same `token` query param and calls the same two endpoints documented below (`GET /api/public/schedule/{token}`, `POST /api/public/reschedule/{token}`) — neither endpoint's own request/response schema changed, only the link format emitted by `send_ical_invitation_email`'s and `upload_resumes`'s callers (`backend/app/routers/public.py`, `backend/app/routers/jobs.py`).
...
- **2026-09-06** — **Fixed a bug where completing a screening interview could silently mark a candidate as advanced to the functional stage.** Screening and functional interviews share **ONE** `InterviewSession` row per applicant (`InterviewSession.id == str(Applicant.id)`); the session is tagged with which stage it was provisioned for via `session.settings['stage']` (set by `sync_applicant_to_ai`, defaulting to `'functional'` only for legacy sessions with no tag). Three call sites that read a session's status/score back onto the `Applicant` row were writing to `functional_status`/`functional_score` **unconditionally**, regardless of that tag — so a candidate who had only ever done a **screening** interview could have their session evaluated and end up with `functional_status="completed"` and a populated `functional_score`, exactly as if a recruiter had clicked "Advance" into the functional stage, when nobody had. Fixed by adding a shared helper `session_stage(session)` (new, `backend/app/utils/ai_sync.py`) that returns `'screening'` or `'functional'` from that tag, and branching on it at all three sites before choosing which pair of columns to write: (1) `_reconcile_stage_from_sessions` in `backend/app/routers/jobs.py` (renamed from `_reconcile_functional_from_sessions`; internal helper, not a route itself, but run on every load of **GET /api/jobs/{job_id}/responses** and **GET /api/jobs/{job_id}/interview-analysis**) now writes `screening_status`/`screening_score` for a screening-stage session and `functional_status`/`functional_score` for a functional-stage one; (2) **POST /api/jobs/webhooks/interview-completed** (`interview_completed_webhook`) applies the same branch instead of always writing the functional pair; (3) **GET /api/usage/candidates-table** (`get_candidates_table`)'s per-applicant sync loop applies the same branch. In all three, fields with no screening/functional split (`report_url`, `proctoring_severity_flag`, `cheat_probability`) remain unconditional — this fix is scoped to the status/score columns only — and `Applicant.overall_interview_score` (in the jobs.py sites) is now only updated when the session's stage is `'functional'`, since it represents the functional interview's score specifically. No request/response schema changed on any of the four affected endpoints.
- **2026-09-06** — **`PATCH /api/jobs/applicants/{applicant_id}` now also regenerates `scheduling_token` when a candidate is un-rejected.** Previously the route only refreshed `scheduling_token` when a `screening_status`/`functional_status` update carried a truthy value (i.e. on a stage advance). It now additionally refreshes the token when the request body includes `decision` and the applicant's decision was `"rejected"` before the update but is not `"rejected"` after it (e.g. set to `null` or any other value). Rationale: rejecting a candidate already revokes their live interview link (expires pending/started `interview_invites` and rotates the engine session's `inviteToken` — documented below since 2026-06-28); un-rejecting on its own does not touch `screening_status`/`functional_status` (those are preserved untouched across a reject/un-reject cycle), so without this fix a restored candidate had no valid token to get back into an interview until their stage was manually re-advanced. `ApplicantUpdateIn` and `ApplicantOut` schemas are unchanged — `scheduling_token` was already returned on `ApplicantOut`, it is just refreshed in one additional case now.
- **2026-09-06** — **New soft-delete/restore pair for applicants: `POST /api/jobs/applicants/{applicant_id}/remove` and `POST /api/jobs/applicants/{applicant_id}/restore`, plus a new `Applicant.removed_at` (nullable timestamp) column.** Distinct from the existing `DELETE /api/jobs/applicants/{applicant_id}` (DSAR-grade anonymise-in-place, which scrubs identifying fields): `/remove` only stamps `removed_at = now()`, leaving every other field — resume text, scores, reports — untouched, so the row remains usable as training data; reversible via `/restore`, which clears `removed_at` back to `null`. Both routes are gated the same as the existing PATCH/DELETE applicant routes (`_verify_applicant_access` — any org collaborator), take no request body, and record a `ComplianceAuditLog` entry (`action="applicant.removed"` / `"applicant.restored"`, actor_type `recruiter`). `/remove` additionally revokes the candidate's live interview access the same way a `decision: "rejected"` PATCH already does: any pending/started `InterviewInvite` rows are set to `expired` and the linked engine `InterviewSession.inviteToken` is rotated, so an already-issued interview link stops working immediately (`_revoke_applicant_invites`, shared helper in `backend/app/routers/jobs.py`); `/restore` does NOT reissue a link — a fresh one is only minted the next time the candidate is (re)advanced through a stage. **New column:** `Applicant.removed_at` (`TIMESTAMP WITH TIME ZONE`, nullable, added via `main.py:init_db()` `ADD COLUMN IF NOT EXISTS`) — when set, the applicant is now excluded from essentially every recruiter-facing read: job funnel/pipeline counts, `GET /api/jobs/{job_id}/responses`, `GET /api/jobs/{job_id}/interview-analysis`, resume-upload dedup matching, the leaderboard, usage-stats endpoints, the platform overview/organisations/jobs/interviews endpoints, and internal talent-finder search. This is a pure read-side filter — no existing response schema gained or lost a field because of it.
- **2026-09-06** — **`POST /api/jobs/{job_id}/applicants/upload-resumes` no longer advances candidates into recruiter screening (or any other interview stage) as a side effect of uploading a resume.** The optional legacy `source` query parameter remains accepted and enum-validated for backwards compatibility, but is ignored for stage routing. A newly created applicant is now always stored with `source="bulk_upload"`, `entry_method="bulk_upload"`, and both `screening_status` / `functional_status` unset (`null`). When a parsed resume matches an existing applicant by email or name, the route updates the resume and eligible placeholder contact details without changing that applicant's existing `source`, `screening_status`, or `functional_status`. Resume upload is intake only; recruiters must explicitly advance a candidate after reviewing the resume. Request and response schemas are otherwise unchanged.
- **2026-09-06** — **Added recruiter-authenticated `POST /api/jobs/applicants/{applicant_id}/resume` and widened the existing CV-presence check in `POST /api/interview/sessions/:id/start`.** The new backend route accepts one `multipart/form-data` field named `file` (`.pdf`, `.docx`, or `.txt`; non-empty; at most 10 MiB), targets the exact applicant id rather than deduplicating by candidate identity, stores the upload, best-effort extracts text, and returns the updated `ApplicantOut`. When that applicant already has an engine `InterviewSession`, the route also sets `session.settings.resumeUploaded = true` and copies usable extracted text to the linked engine `Candidate.resumeText`; this refreshes resume state without rerunning the full sync and resetting an in-progress session. Accordingly, `/api/interview/sessions/:id/start`'s `requireCv` gate now passes when **either** the linked engine `Candidate.resumeText` is a non-empty string after trimming **or** `session.settings.resumeUploaded === true`; it returns the existing **400** `CV_REQUIRED` response only when both signals are absent. This lets a real uploaded file satisfy the requirement even when a scanned or otherwise non-extractable document yields no text. The `/start` request and success response shapes are unchanged.
- **2026-09-06** — **New `POST /api/auth/clear-context` (super_admin-only "leave organisation" action), and `GET /api/auth/me` now stops leaking a silently-fallback-resolved org for super_admins.** `clear_context` (`backend/app/routers/auth.py`) is the reverse of `switch-context`: gated by `Depends(require_super_admin)` (**403** `"Only Super Admins can access this."` for non-super_admins), it sets `current_user.last_active_org_id = None`, commits, then `response.delete_cookie(key="active_org_id", path="/")` and returns `{"message": "Cleared organisation context."}` — a full reset (not just the browser's cookie) so a future login doesn't silently re-scope into the old org. Separately, `UserProfileOut` gains `active_org_explicit: bool` (default `False`). In `get_me()`, for a super_admin this is `True` only when the active org came from an explicit choice — the `active_org_id` cookie or the durable `users.last_active_org_id` — as opposed to `get_active_org_id()`'s silent-fallback tiers (the super_admin's own owned org, or an arbitrary first-created org); when `False` for a super_admin, `organisation_id`/`organisation_name` in the response are now also `null` (previously they showed whatever the silent fallback resolved to). For non-super_admin users, `active_org_explicit` is always `False` and does not affect `organisation_id`/`organisation_name` at all — their resolution is unchanged.
...
```json
{
  ...
  "active_org_explicit": "boolean"
}
```
response_model = `UserProfileOut` (Config.from_attributes=True). designation/organisation_id/organisation_name optional, default None. `google_drive_connected` defaults False. `active_org_explicit` defaults False.

Status codes: 200 OK; 401 Unauthorized — get_current_user failures.

Notes: For super_admin, org_id resolved via get_active_org_id. **(2026-06-30)** Its super_admin fallback chain is now: `active_org_id` cookie → the user's stored internal `users.last_active_org_id` → the super_admin's own `organisation_id` → deterministic first org (`ORDER BY created_at ASC, id ASC`) — previously it was just cookie → arbitrary `.first()`. For non-super_admin with organisation_id set, organisation_name looked up from current_user.organisation_id. `onboarding_required` = (resolved org_id is None) AND (user_type != super_admin). **(2026-09-06)** `google_drive_connected = bool(current_user.google_refresh_token)` — true once the user has completed the Google OAuth connect flow (recordings are uploaded to their own Drive); does not verify the token is still valid, only that one was ever stored. **(2026-09-06)** `active_org_explicit`: for a super_admin, `True` only if the active org was resolved from an explicit choice — a valid `active_org_id` cookie, or the durable `users.last_active_org_id` — as opposed to a silent fallback (the super_admin's own owned org, or an arbitrary first-created org via `get_active_org_id()`'s later tiers). When `False` for a super_admin, `organisation_id` and `organisation_name` are forced `null` in the response (the silent-fallback org is no longer resolved/returned at all). For non-super_admin users, `active_org_explicit` is always `False` and has no effect on `organisation_id`/`organisation_name`, which resolve exactly as before.
...
#### POST /api/auth/clear-context

Super Admin only: reverse of `switch-context` — clears the active organisation context (durably, not just the cookie), returning the super_admin to the neutral "Platform" state.

- **Auth:** JWT httpOnly cookie (required); role gate: `Depends(require_super_admin)` (else 403).
- **Path params:** none
- **Query params:** none
...
```json
{
  "message": "Cleared organisation context."
}
```

Status codes: 200 OK; 401 Unauthorized — get_current_user failures; 403 Forbidden — "Only Super Admins can access this." (require_super_admin).

Notes: **(2026-09-06)** Sets `current_user.last_active_org_id = None` and commits, then `response.delete_cookie(key="active_org_id", path="/")` — a full reset rather than just clearing the cookie, so a future login doesn't silently re-resolve back into the cleared org via the durable `last_active_org_id` fallback. After this call, `GET /api/auth/me` returns `active_org_explicit: false` and `organisation_id`/`organisation_name`: null for this super_admin until they explicitly choose an org again (via `switch-context` or the org-switcher UI).
...
Parse an uploaded PDF/DOCX/TXT job description and extract structured job metadata via LLM (DeepSeek → Groq → Grok → Gemini), falling back to source-derived title/raw text with empty authored assessment fields when no provider succeeds.
...
Notes: Plain dict (no response_model); fixed key set but nested parameter shapes may vary by provider output. All provider prompts require every criterion/topic/question to be traceable to the uploaded JD or explicit `prompt` field and prohibit assuming an occupation from a broad title such as "engineer". If every provider fails or no key is configured, `role_name`/`card_name` are extracted from a labelled title, the first plausible source line, or filename; `description` is the raw extracted text; `experience_band`/`skills` are empty strings; question arrays are empty; `resume_parameters` contains empty `must_have`/`red_flags`/`good_to_have`; `screening_parameters` is `{}`; and `functional_parameters` is `{ "topics": [] }`. The no-key path still includes the existing 1.2s delay. LLM keys from env: DEEPSEEK_API_KEY, GROQ_API_KEY, GROK_API_KEY/XAI_API_KEY, GEMINI_API_KEY.
...
Notes: Excludes test/sentinel applicants. `_reconcile_stage_from_sessions` (renamed from `_reconcile_functional_from_sessions`) may mutate applicants (mark completed, copy score/report_url) before building the response. Non-overview tabs serialize raw ORM Applicants, not strictly ApplicantOut. **(2026-09-06)** This reconciliation is now stage-aware: screening and functional interviews share one `InterviewSession` row per applicant, and the helper checks `session_stage(session)` before writing — a session provisioned for screening now updates `screening_status`/`screening_score`, and only a session provisioned for functional updates `functional_status`/`functional_score` (previously it always wrote the functional pair, so completing a screening-only interview could incorrectly mark the applicant as advanced to functional).
...
- **Auth:** JWT httpOnly cookie (required) + _verify_job_access.
- **Path params:** `job_id`:UUID — the job id
- **Query params:** `source`:ApplicantSource (optional) — `career_page|bulk_upload|direct_link|scheduled|ats|functional|exit`. Drives stage placement for **newly-created** applicants only, same mapping as `POST /{job_id}/applicants`/`POST /{job_id}/applicants/bulk`: `scheduled` → `screening_status=pending`; `functional` or `exit` → `functional_status=pending`; omitted or any other value → defaults to `bulk_upload`, both stage-status fields stay `null`. Has no effect when the parsed resume matches an existing applicant (see Notes).
...
Status codes: 200 OK; 401 not authenticated; 403 'Access denied'; 404 'Job not found'; 422 missing `files` field or invalid legacy `source` enum value.

Notes: `response_model=List[ApplicantOut]`. Saves to `uploads/resumes/`. `DEEPSEEK_API_KEY` is read from env for `parse_resume_with_deepseek`. Deduplication matches an existing applicant first by lowercase email (ignoring `@candidate.io` dummies), then by lowercase name (ignoring `Candidate`). For a match, the route updates `resume_url`, usable `resume_text`, and only placeholder/unset name, email, or phone values; it does **not** mutate the existing applicant's `source`, `screening_status`, or `functional_status` — the `source` query param has no effect on this branch. For a genuinely new applicant, the route gets email fallback `candidate.<hex>@candidate.io`, phone `+1 555-0199`, `entry_method="bulk_upload"`, `resume_analysed=false`, and `source` set from the query param (mapped to stage status as described above) or defaulted to `"bulk_upload"` (both `screening_status`/`functional_status` stay `null`) — same source→stage mapping as `add_applicant`/`add_applicants_bulk` (fixed 2026-09-07; previously the query param was accepted but ignored here, always forcing `bulk_upload`/Resume Analysis). Broadcasts a WebSocket update.

#### POST /api/jobs/applicants/{applicant_id}/resume

Attach or replace the resume for one exact existing applicant, persist any usable extracted text, and refresh the already-provisioned engine candidate/session resume state without recreating the interview session.

- **Auth:** JWT httpOnly cookie (required) + `_verify_applicant_access` (resolves the applicant's job and applies `_verify_job_access`: a super_admin must have that job's organisation selected as the active organisation; an org_admin must belong to it; a member must both belong to it and be a job collaborator).
- **Path params:** `applicant_id`:UUID — the applicant id
- **Query params:** none

Request: `multipart/form-data`
```
file: UploadFile (required) — one .pdf, .docx, or .txt file; must be non-empty and at most 10 MiB (10 * 1024 * 1024 bytes)
```
...
// ApplicantOut
{
  ...
  "email": "string",
  "phone": "string|null",
  "source": "career_page|bulk_upload|direct_link|scheduled|ats|functional|exit|null",
...
Status codes: 200 OK; 400 `{"detail":"Resume must be a PDF, DOCX, or TXT file."}` for an unsupported or extensionless filename; 400 `{"detail":"Resume file is empty."}` for a zero-byte file; 400 `{"detail":"Invalid resume filename."}` when the resolved storage path is unsafe; 401 not authenticated; 403 `{"detail":"Access denied"}` when the authenticated user cannot access the applicant's job; 404 `{"detail":"Applicant not found"}` / `{"detail":"Job not found"}`; 413 `{"detail":"Resume must be 10 MB or smaller."}`; 422 FastAPI validation error when `applicant_id` is not a UUID or the required multipart `file` field is missing.

Notes: `response_model=ApplicantOut`. The original client filename is reduced to `os.path.basename(...)`; the stored path is `uploads/resumes/<applicant_id>-<filename>` after `_safe_upload_path` validation. `extract_text_from_file(...)` is best-effort: extraction errors do not fail the request, and extracted text shorter than 50 characters is treated as unusable. `applicant.resume_url` is always updated after a valid upload; `applicant.resume_text` is updated only when usable text was extracted, so an existing non-empty value is not erased by an unextractable replacement. If an engine `InterviewSession` with `id == str(applicant.id)` already exists, its free-form JSON `settings` is merged with `resumeUploaded: true`; when usable text exists, the linked engine `Candidate.resumeText` is also updated. The route commits these changes in the shared database and returns the refreshed applicant. It does not rerun `sync_applicant_to_ai`, does not reset the interview session, and does not broadcast a WebSocket update.
...
Notes: response_model=ApplicantOut. If screening_status or functional_status provided and truthy: regenerates scheduling_token (uuid4) and calls sync_applicant_to_ai. Broadcasts WebSocket update. **(2026-06-28)** When the update sets `decision == "rejected"`, auto-revokes the candidate's live interview links: all pending/started `interview_invites` rows are set to expired, and the engine `InterviewSession.inviteToken` is rotated to a fresh, never-shared value so any saved room URL stops working (request/response schema unchanged). **(2026-09-06)** `scheduling_token` is also regenerated on un-reject: if the request body includes `decision` and the applicant's decision was `"rejected"` before this update but is not `"rejected"` after it, a fresh `scheduling_token` is minted even though neither `screening_status` nor `functional_status` changed (rejection never clears those fields, so un-rejecting alone wouldn't otherwise trigger a token refresh, leaving a restored candidate with no valid link back into an interview).
...
#### POST /api/jobs/applicants/{applicant_id}/remove

Soft-delete an applicant: hides the candidate from recruiter-facing lists/counts/search without scrubbing any field, and revokes their live interview link.
...
Response:
```json
{ "message": "Candidate removed.", "applicant_id": "string (uuid)" }
```

Status codes: 200 OK; 401 not authenticated; 403 'Access denied'; 404 'Applicant not found' / 'Job not found'.

Notes: Plain dict response. Sets `Applicant.removed_at = now()`. Unlike `DELETE /api/jobs/applicants/{applicant_id}` above (which anonymises/scrubs the row for DSAR purposes), this does NOT null or alter any other field — resume text, scores, and reports all stay intact; the applicant simply drops out of recruiter-facing reads (see the 2026-09-06 changelog entry above for the full list of filtered endpoints) until restored. Also expires any pending/started `InterviewInvite` rows and rotates the linked engine `InterviewSession.inviteToken`, so an already-issued interview link stops working (same mechanism the existing PATCH `/api/jobs/applicants/{applicant_id}` already runs when `decision` is set to `"rejected"`). Records a `ComplianceAuditLog` entry (`action="applicant.removed"`). Reversible via `POST /api/jobs/applicants/{applicant_id}/restore` below.

#### POST /api/jobs/applicants/{applicant_id}/restore

Undo a `/remove`: clears the soft-delete flag so the candidate reappears in every recruiter-facing list/count/search.
...
Response:
```json
{ "message": "Candidate restored.", "applicant_id": "string (uuid)" }
```

Status codes: 200 OK; 401 not authenticated; 403 'Access denied'; 404 'Applicant not found' / 'Job not found'.

Notes: Plain dict response. Clears `Applicant.removed_at` back to `null`. Does NOT restore any previously-revoked invite link — a fresh one is only issued the next time the candidate is (re)advanced through a stage (e.g. via `POST /api/jobs/applicants/{applicant_id}/schedule`). Records a `ComplianceAuditLog` entry (`action="applicant.restored"`).
...
```json
{
  ...
  "recordingPlaybackUrl": "string | null",     // freshly-presigned B2 URL for InterviewSession.recordingB2Key; null if that column is unset
  "recordingStartedAt": "string | null",       // ISO-8601, InterviewSession.startedAt (UTC-normalized); null if never started
...
Notes: No response_model; delegates with str(applicant_id). `report` is `null` until the engine scores the interview (`evaluated=false`). The `report` payload is the canonical `CandidateReport` defined by the dashboard and is served VERBATIM (no reshaping) from `InterviewSession.evaluation`. See `app/utils/ai_sync.py:get_applicant_full_report`. **(2026-09-06)** `recordingUrl`/`recordingDriveFileId`/`transcript` added to BOTH branches above — lets Interview Analysis render a real video/transcript panel straight from this one endpoint, since both the recording and the raw transcript already live as columns on the same `InterviewSession` row (no separate endpoint needed). **(2026-09-06)** `recordingPlaybackUrl`/`recordingStartedAt` added to BOTH branches — recordings now upload to Backblaze B2 (private bucket) instead of Google Drive, so playback needs a per-request presigned URL (`get_presigned_recording_url`, `backend/app/utils/backblaze.py`, TTL `B2_PRESIGNED_URL_TTL_SECONDS`, default 14400s/4h) minted fresh from `InterviewSession.recordingB2Key`; `recordingStartedAt` gives the dashboard the recording's t=0 to place proctoring-violation markers on the video timeline. `recordingUrl`/`recordingDriveFileId` stay in place unchanged as a backward-compat fallback for pre-switch recordings.
...
Job-level Interview Analysis listing: every applicant whose AI interview has been EVALUATED, with a compact summary read from the stored evaluation. A reconcile pass (`_reconcile_stage_from_sessions`, renamed from `_reconcile_functional_from_sessions`) first persists scores/proctoring/InterviewReport autonomously, so the list reflects a finished interview immediately — no manual transcript→LLM→upload step. The full report is fetched per-row on open via **GET /api/jobs/applicants/{applicant_id}/functional-report**.
...
Notes: Plain dict response. No get_current_user dependency. proctoring_severity_flag derived from ProctoringLog.severity (critical/high/medium/low); cheat_probability mapped from flag (critical|high→high, medium→medium, else low) and report_url=session.reportUrl are set unconditionally. Upserts InterviewReport(summary, transcript, video_url, detailed_scores). **(2026-09-06)** Screening and functional interviews share one `InterviewSession` row per applicant, so which pair of columns the score/status get written to now depends on `session_stage(session)`: if the session was provisioned for the **functional** stage, `overall_interview_score=functional_score=evaluation.overallScore` and `functional_status=completed`; if it was provisioned for **screening**, only `screening_score=evaluation.overallScore` and `screening_status=completed` are set (`overall_interview_score` is left untouched). Previously this route always wrote the functional pair regardless of stage, so completing a screening interview could incorrectly mark the applicant as advanced to functional — fixed by the new `session_stage()` helper in `backend/app/utils/ai_sync.py`. **(2026-06-27)** Also marks the applicant's most-recent non-completed `interview_invites` row status=completed + completed_at in the same transaction (request/response schema unchanged).
...
Flat list of all candidates (applicants) across the user's visible jobs. As a side effect it lazily re-syncs each applicant's screening or functional stage (whichever the matching InterviewSession was provisioned for) from that InterviewSession (status, score, report_url, cheat_probability, attempted_at) and commits changes before returning.
...
Notes: Side effects (writes to DB): for each applicant whose id matches an InterviewSession.id, may update status (EVALUATED→completed, IN_PROGRESS→scheduled) and score (session.evaluation['overallScore']) — **(2026-09-06)** on `screening_status`/`screening_score` if the session's `session_stage(session)` is `'screening'`, or on `functional_status`/`functional_score` if it is `'functional'` (previously this loop always wrote the functional pair regardless of which stage the session was actually for, so a screening-only interview could incorrectly mark the applicant as advanced to functional; fixed via the new `session_stage()` helper in `backend/app/utils/ai_sync.py`). report_url, cheat_probability (high if any CRITICAL/HIGH ProctoringLog severity, medium if any MEDIUM, else low), and attempted_at (session.completedAt or updatedAt) remain unconditional — not split by stage. A single db.commit() runs if any applicants exist. Enum fields emitted as raw .value strings. **Test-session applicants (`remarks == "__ih_test_session__"`) are excluded** from the returned list (mirrors the same exclusion on /stats and the jobs.py funnel/roster/analytics).
...
```json
{
  ...
  "skip_notification": "boolean"    // optional, default false (2026-09-07) — see Notes
}
```
...
```json
{
  ...
  "applicant_id": "string",         // (2026-09-07) str(applicant.id) — also the engine InterviewSession id
  "job_id": "string | null"         // (2026-09-07) str(applicant.job_id), or null if the applicant has none
}
```
...
Notes: Mutates and commits the Applicant: functional stage sets functional_scheduled_at=parsed_time, functional_status=scheduled, sync_applicant_to_ai (non-fatal); screening stage sets screening_scheduled_at=parsed_time, screening_status=scheduled. Always increments calendar_sequence = (calendar_sequence or 0) + 1. If calendar_event_id exists, calls update_calendar_event (non-fatal). Resends send_ical_invitation_email (duration_minutes=30, sequence=calendar_sequence; non-fatal), UNLESS `skip_notification` is true. Unlike /confirm, does NOT raise if no stage status is set. **(2026-09-05)** Immediately after the iCal email send, also fires a best-effort **WhatsApp confirmation** (`send_schedule_confirmation_whatsapp`, `app/utils/twilio_client.py`) in its own try/except — failure never affects the email or this response. No-ops when Twilio isn't configured or `applicant.phone` isn't a real number. **(2026-09-06)** Added an upper bound on `new_time`: previously only a past date was rejected, so a stray or malicious far-future date (e.g. year 3000) was accepted outright with no recruiter-configured scheduling window to check against; now rejects anything more than 60 days out (fixed safety rail, not a business rule), on top of the existing "must be in the future" check. **(2026-09-07)** Added `skip_notification` (request) and `applicant_id`/`job_id` (response): `app/reschedule`'s new "Start interview now" action reschedules to the current instant and redirects straight into the interview room — sending "your interview has moved" email/WhatsApp confirmations in that flow is pointless since the candidate is joining immediately, so `skip_notification: true` skips BOTH the iCal email and the WhatsApp send (the two calls above are now wrapped in `if not skip_notification`), and `applicant_id`/`job_id` let the client build `{ENGINE_WEB_URL}/interviewcandidateroom?sessionId={applicant_id}&jobId={job_id}` without a second round trip. Existing callers that omit `skip_notification` are unaffected (defaults false, notifications still sent as before) and can ignore the two new response fields.
...
#### POST /api/public/interview-session/{session_id}/recording

Server-to-server call from the interview engine after a recording capture finishes: uploads the file to the Backblaze B2 recordings bucket (`upload_interview_recording`).

- **Auth:** optional shared-secret header `X-Webhook-Secret`, checked against `settings.WEBHOOK_SECRET` when that setting is non-empty (permissive — no check — when unset). Same pattern as the screening-outcome webhook below.
- **Path params:** `session_id`:str — the Applicant id (UUID) for a real scheduled interview, or the engine's default Prisma CUID for a demo/test-room session with no matching `Applicant` row.
- **Query params:** none
...
Response (B2 configured and upload succeeds):
```json
{ "ok": true, "b2Key": "string" }   // e.g. "recordings/{session_id}/{candidate name} - {job title} - {session_id}.webm"
```
Response (B2 credentials not configured, or the upload fails — `upload_recording_to_b2` returns `None`):
```json
{ "ok": false, "simulated": true }
```

Status codes: 200 OK (both success and simulated-failure cases); 403 "Invalid webhook secret." (secret configured and mismatched).

Notes: `backend/app/utils/backblaze.py:upload_recording_to_b2` (boto3 S3-compatible client against `B2_ENDPOINT`/`B2_BUCKET_NAME`). The uploaded file is copied to a temp file first (`tempfile.NamedTemporaryFile`, cleaned up in a `finally`), then pushed to B2 under key `recordings/{session_id}/{filename}` where `filename` is `"{candidate name} - {job title} - {session_id}.webm"` (falls back to `str(session_id)` for the candidate name and `"Interview"` for the job title when no `Applicant` row matches). This route does **not** itself write to `InterviewSession` — the caller (engine) persists the returned `b2Key` onto `InterviewSession.recordingB2Key`. **(2026-09-06)** Switched from Google Drive (`app/utils/google_drive.py:upload_recording`) to Backblaze B2 — the success response changed from `{"ok": true, "driveFileId": string, "driveUrl": string}` to `{"ok": true, "b2Key": string}`; `driveFileId`/`driveUrl` are no longer returned. The `{"ok": false, "simulated": true}` failure-path shape is unchanged.
...
Generates job-description-grounded interview questions for a job role using DeepSeek (falls back to deterministic questions derived from that request's JD/role if the AI call fails), persists up to the first 10, and returns the created Question rows.
...
```
{
  ...
  roleType: string,         // required (roleApplicability enum value; not used as a preset competency map)
...
Notes: generateQuestions calls DeepSeek with a domain-neutral, source-grounded prompt. On any error it splits `jobDescription` into usable clauses (up to 8) and creates one role-specific evidence question/rubric per clause; if no clause is usable it creates three questions derived from `jobTitle`/`roleType`. This preserves the zero-key path without injecting a preset occupation or competency bank. Only the first 10 generated questions are persisted (`slice(0,10)`) inside a prisma.$transaction. aiEvaluationGuidance is stored as a stringified JSON blob.
...
Lightweight manual-testing harness for the real-time voice pipeline (director + LiveKit + `AgentAudioVisualizerAura`), reached via the standalone `/voice-test` page — bypasses the full candidate-room consent/permission/proctoring flow entirely. Reuses the exact same demo company/role/questions as `GET /demo-session`.
...
Notes: No Fastify schema. Calls `getOrCreateDemoSession()` and returns `{ sessionId: session.id }` directly — no settings are read or written (since voice/LiveKit is now the only interview mode, there is no `conversationalInterview` flag to force on anymore; see 2026-09-06 changelog entry). The caller still needs to call `POST /api/interview/sessions/:id/start` and then `POST /api/interview/sessions/:id/livekit-token` itself — this route only prepares the session, it doesn't start the call.
...
200 OK (application/json)
{
  ...
  initialQuestion: string,     // first active authored question, or a fallback derived from the session's job role/JD
